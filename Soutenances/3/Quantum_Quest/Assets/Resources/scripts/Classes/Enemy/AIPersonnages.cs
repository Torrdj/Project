...
public class AIPersonnages : MonoBehaviour
{
    ...
    public void FixedUpdate()
    {
        ...
        if (attaquant != -1)
        {
            if(!isDead && !IsParalysed && isLoad && checkMana(5))
            {
                updateManaRPC(5);
                coupDeMolette(attaquant);
    ...
    {
        attaquant = GameObject.FindGameObjectWithTag("Player").GetComponent<PhotonView>().viewID;
        GetComponent<AI_Movements>().attaquant = attaquant;
